async function loadData() {
  try {
    ...
    // Home team on top half
    spreadFormation(pitch, data.starters.home, 20, "home");
    // Away team on bottom half
    spreadFormation(pitch, data.starters.away, 80, "away");
...
function spreadFormation(pitch, players, baseY, side) {
  const stepX = 100 / (players.length + 1);
  players.forEach((p, i) => {
    addPlayer(pitch, p, (i + 1) * stepX, baseY, side);
  });
}
